...
description:
    ...
    It contains the code for the chat interface and the logic to interact with the OpenAI API.
"""
import time
import os
import json
...
def get_stock_price(ticker):
    """
    This function returns the latest stock price for a given ticker symbol.
    """
...
def handle_function(a_run):
    """
    This function handles the required action of a run.
    """
    tools_to_call = a_run.required_action.submit_tool_outputs.tool_calls
    tools_output_array = []
...
st.sidebar.markdown(f"Get an Open AI Access Key [here]({URL}). ")
if openai_api_key:
...
def process_message_with_citations(orignal_message):
    """
    This function processes the assistant's messages and replaces citations with the actual content.
    """
    message_content = orignal_message.content[0].text.value
   ...
   # st.write(getStockPrice('AAPL'))
    ...
    if prompt := st.chat_input("How can I help you?"):
        ...
        run = client.beta.threads.runs.create(
            ...
            instructions="""
                Please answer the queries using the knowledge provided in the files.
